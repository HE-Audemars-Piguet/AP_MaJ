    ...
    public partial class MainWindow : ThemedWindow, INotifyPropertyChanged
    {
        ...
        private void ShowHelp_Click(object sender, DevExpress.Xpf.Bars.ItemClickEventArgs e)
        {
            ...
            try
            {
                // Login to Vault
                // =====================================
                VaultUtility vltUtil = new VaultUtility();
                vltUtil.VaultConnection = vltUtil.ConnectToVault(AppOptions);
                ...
                long NamePropId = vltUtil.VaultConnection.PropertyManager.GetPropertyDefinitions(VDF.Vault.Currency.Entities.EntityClassIds.Files, null, VDF.Vault.Currency.Properties.PropertyDefinitionFilter.IncludeAll | VDF.Vault.Currency.Properties.PropertyDefinitionFilter.OnlyActive).Where(x => x.Key == "Name").FirstOrDefault().Value.Id;
                List<SrchCond> SearchList = new List<SrchCond>();
                SearchList.Add(new SrchCond()
                {
                    PropDefId = NamePropId,
                    PropTyp = PropertySearchType.SingleProperty,
                    SrchOper = 3,
                    SrchRule = SearchRuleType.Must,
                    SrchTxt = "AXEXX000330.ipt"
                });

                ACW.File[] files = vltUtil.VaultConnection.WebServiceManager.DocumentService.FindFilesBySearchConditions(SearchList.ToArray(), null, null, true, true, ref bookmark, out status);
                if (files == null)
                {
                    res += "Unable to find file" + System.Environment.NewLine;
                    
                }
                else if (files.Length == 1)
                {
                    currentFile = files.FirstOrDefault();
                    res += "File found '"+ currentFile.Name + "' (MasterId='" + currentFile.MasterId + "')" + System.Environment.NewLine;
                }
                else
                {
                    res += "Multiple files found" + System.Environment.NewLine;
             
                }

                // Get file from Vault
                // =====================================
                VDF.Vault.Settings.AcquireFilesSettings AcquireSettings = new VDF.Vault.Settings.AcquireFilesSettings(vltUtil.VaultConnection);
                ...
                ACW.File File = vltUtil.VaultConnection.WebServiceManager.DocumentService.GetLatestFilesByMasterIds(new long[] { currentFile.MasterId }).FirstOrDefault();

                AcquireSettings.AddFileToAcquire(new VDF.Vault.Currency.Entities.FileIteration(vltUtil.VaultConnection, File), VDF.Vault.Settings.AcquireFilesSettings.AcquisitionOption.Download);

                VDF.Vault.Results.AcquireFilesResults AcquireResults = vltUtil.VaultConnection.FileManager.AcquireFiles(AcquireSettings);

                if (!AcquireResults.IsCancelled)
                {
                    foreach (FileAcquisitionResult result in AcquireResults.FileResults)
                    {
                        res += "File '" + result.LocalPath.FullPath + "' has been acquired";

                        if (result.Status == FileAcquisitionResult.AcquisitionStatus.Exception) res += " (State='" + result.Status + "' error code '" + result.Exception.Message + "')" + System.Environment.NewLine;
                        else if (result.Status == FileAcquisitionResult.AcquisitionStatus.Restriction) res += " (State='" + result.Status + "' restrictions '" + result.Restrictions.ToString() + "')" + System.Environment.NewLine;
                        else res += " (State='" + result.Status + "')" + System.Environment.NewLine;
                    }
                }
                else
                {
                    res += "File acquire canceled after 10s." + System.Environment.NewLine;
                }

            }
            catch (Exception Ex)
            {
                res += "Exception:" + System.Environment.NewLine;
                res += "=============================================" + System.Environment.NewLine;
                res += Ex.ToString() + System.Environment.NewLine;
            }

            System.IO.File.WriteAllText(@"C:\Temp\Ahhhh.log", res);
        }
        #endregion
