...
namespace Ch.Hurni.AP_MaJ
{
    ...
    public partial class MainWindow : ThemedWindow, INotifyPropertyChanged
    {
        ...
        {
            List<string> ColList = Data.Tables["Entities"].Columns.Cast<DataColumn>().Where(x => !x.ColumnName.EndsWith("Id") && !x.ColumnName.StartsWith("Vault") && !x.ColumnName.Equals("State") && !x.ColumnName.Equals("Task")).Select(x => x.ColumnName).ToList();
